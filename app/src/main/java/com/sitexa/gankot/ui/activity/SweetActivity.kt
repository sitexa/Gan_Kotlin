...
import com.sitexa.gankot.app.media_image_height
import com.sitexa.gankot.app.media_image_width
import com.sitexa.gankot.app.sitexaApiBaseUrl
import com.sitexa.gankot.net.SweetApi
import com.sitexa.gankot.net.glideHeader
...
import com.sitexa.gankot.utils.testDimen
import com.sitexa.gankot.utils.toast
import io.reactivex.android.schedulers.AndroidSchedulers
import io.reactivex.schedulers.Schedulers
import kotlinx.android.synthetic.main.activity_sweet.*
import kotlinx.android.synthetic.main.activity_sweet_content.*
...
class SweetActivity : AppCompatActivity() {
    ...
    private fun calcDemin() {
        val p = testDimen()
        media_image_width = (p.first - 40) / 3
        media_image_height = (media_image_width * 3) / 4
    }
            ...
            if (holder is HeadHolder) {
                ...
                if (!medias.isEmpty())
                    for (i in 0..medias.size - 1) {
                        val media = medias[i]
                        val imgUri = sitexaApiBaseUrl + "/media/${media.fileName}/${media.fileType}"
    ...
    internal class HeadHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            val lp = LinearLayout.LayoutParams(media_image_width, media_image_height)
